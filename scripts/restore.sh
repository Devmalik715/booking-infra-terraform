#!/usr/bin/env bash
#
# Restores a dump into a *fresh* database and then proves the restore worked.
#
#   ./scripts/restore.sh                                   # newest dump -> new bookings_restore_<stamp> db
#   ./scripts/restore.sh backups/bookings_20260910_213045.dump
#   ./scripts/restore.sh -d bookings_check                 # pick the target database name
#   ./scripts/restore.sh -d bookings --force               # overwrite the live database (asks first)
#
# The restore goes into a brand new database by default. That is the point: the
# original data stays untouched, so the two can be compared row for row, which
# is the only way to know a backup is actually restorable.
...
# No dump given: take the newest one in the backup directory.
if [[ -z "${DUMP_FILE:-}" ]]; then
    # shellcheck disable=SC2012  # names are generated by backup.sh, no spaces or newlines in them
...
# If a checksum was written alongside the dump, use it. A backup that restores
# from a silently corrupted file is worse than no backup at all.
...
# pg_restore reads the archive from stdin. --exit-on-error means a partial
# restore fails loudly instead of leaving a half-populated database behind.
...
# pg_dump does not carry planner statistics, so a freshly restored database
# plans queries from defaults until something analyses it. Skipping this is a
# classic reason a restored database "works but is slow".
psql_run "${TARGET_DB}" "VACUUM ANALYZE" >/dev/null
echo "    statistics rebuilt (VACUUM ANALYZE)"

# ---------------------------------------------------------------------------
# Verification. Row counts alone are weak - they pass even if every amount came
# back as NULL - so the sum of amount and the newest created_at are compared as
# well, plus the indexes that Part 5 depends on.
# ---------------------------------------------------------------------------
